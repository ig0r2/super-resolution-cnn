...
from videoplayer.evaluator_perf_video_cv2 import EvaluatorPerfVideoCV2, Runtype

# Speed evaluation for the OpenCV-decode video player pipeline (videoplayer/), supporting the
# tensorrt / onnxruntime-* / ncnn-vulkan backends. One CSV row per (model, runtype) with
# per-resolution, per-stage columns ("<label> decode/sr/e2e/full"), upserted after every
# resolution so results persist incrementally.

STAGES = ("decode", "sr", "e2e", "full")

if __name__ == "__main__":
    UPSCALE_FACTOR: Literal[2, 3, 4] = 2
    RUNTYPE: Runtype = "ncnn-vulkan"  # tensorrt | onnxruntime-cuda | onnxruntime-tensorrt | ncnn-vulkan | ...
    ...
    for checkpoint_path in CHECKPOINT_PATHS:
        ...
        log_path = get_logs_path(f"evaluation/eval_video_{checkpoint_path.stem}.txt")
