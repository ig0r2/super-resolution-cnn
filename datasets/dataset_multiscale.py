...
class ImageDatasetMultiscaleTest(data.Dataset):
    ...
    def _transform(self, lr_2, lr_3, lr_4, hr, index):
        ...
        lr_2 = ensure_rgb(lr_2)
        lr_3 = ensure_rgb(lr_3)
        lr_4 = ensure_rgb(lr_4)
        hr = ensure_rgb(hr)
        ...
        lr_2 = crop_to_match(lr_2, target_h // 2, target_w // 2)
        lr_3 = crop_to_match(lr_3, target_h // 3, target_w // 3)
        lr_4 = crop_to_match(lr_4, target_h // 4, target_w // 4)

        # 100 images, quality 20-100
        if self.jpeg_degradation:
            quality = jpeg_quality_for_index(index)
